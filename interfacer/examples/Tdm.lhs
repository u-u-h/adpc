...
grammar[rnastruct]{
   ...
   motif0 = stem0 ... h;
   stem0 = tabulated ( ((sr <<< lbase -~~ ((sr <<< lbase -~~ ((sr <<< lbase -~~ maxstem0 ~~- lbase) `with` basepairing)  ~~- lbase) `with` basepairing)  ~~- lbase) `with` basepairing) 	... h) ;
     
   maxstem0 = (sr <<< lbase -~~ ( motif1 |||
                                (sr <<< lbase -~~ ( motif1 |||
                                   (sr <<< lbase -~~ ( motif1 |||
                                      (sr <<< lbase -~~ ( motif1 |||
                                         (sr <<< lbase -~~ motif1 ~~- lbase) `with` basepairing)
                                          ~~- lbase) `with` basepairing)
                                       ~~- lbase) `with` basepairing)
                                    ~~- lbase) `with` basepairing)
                                 ~~- lbase) `with` basepairing	... h;
   motif1 = bulge1 ... h;
   bulge1 = tabulated (( (bl <<< lbase -~~ region ~~~ motif_b2 ~~- lbase) `with` basepairing)	... h);
   motif_b2 = (sr <<< lbase -~~ motif2 ~~- lbase) `with` basepairing ... h;

   motif2 = stem2 ... h;
   stem2 = tabulated ( 
                (sr <<< (iupac_base 'y') -~~ stem2 ~~- lbase |||
                 sr <<< lbase -~~ motif3 ~~- lbase) `with` basepairing	... h);

   motif3 = hairpin3 ... h;
   hairpin3 = (hl <<< lbase -~~ (region `with` (contains_region "acgu")) ~~- lbase) `with` basepairing ... h;
