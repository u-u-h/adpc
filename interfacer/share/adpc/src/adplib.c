...
char *calc_contains_region(char *z, int n, char *pat)
{
  ...
  arr = (char *) malloc((((n*(n+1))/2) +n+1) * sizeof(char));

  if (adplib_debug>1) printf("calculating...\n");
  
  for (j=0; j<=n; j++) {
    for (i=0; i<=j; i++) {
      arr[(j*(j+1))/2  +i]=0;  // tab(i,j)
      ppos = 0;
      inPattern = 0;
      if (j-i >= l) {
        for (k=i+1;k<=j;k++) {
